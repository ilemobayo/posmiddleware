...
import com.elarasolutions.applications.posmiddleware.utility.comms.Transaction;
import com.elarasolutions.applications.posmiddleware.utility.comms.tcp.TcpComms;
import com.elarasolutions.applications.posmiddleware.utility.comms.tcp.impl.TcpCommsImpl;
...
//        byte[] dataBytes = new Utils().echoRequest();
    ...
    public POSDataPackedResponse sendIsoRequestNew(POSDataPacked data) throws Exception {
        byte[] dataBytes = data.getMsg().getBytes();
//        byte[] dataBytes = new Utils().echoRequest();
        /*
         * Convert iso data gotten from the request body to POSData
        ...
        try {
            IsoMessage isoResponseMessage = Transaction.sendRequest(data.getHost(), data.getPort(), data.isSsl(), dataBytes);
            return buildISOJsonDataNew(isoResponseMessage);
        } catch (Exception ex) {
            ex.printStackTrace();
    ...
    private POSDataPackedResponse buildISOJsonDataNew(IsoMessage message){
        POSData posData = new POSData();
        for (int i = 0; i <= 128; i++) {
            if (message.hasField(i)) {
                if (i == 2) {
        ...
        POSDataPackedResponse posDataPackedResponse = new POSDataPackedResponse();
        posDataPackedResponse.setMsg(message.debugString());
        return posDataPackedResponse;
    }
}
